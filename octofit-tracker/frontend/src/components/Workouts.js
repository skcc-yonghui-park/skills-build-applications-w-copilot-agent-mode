...
function Workouts() {
  ...
  async function loadWorkouts(signal) {
    try {
      ...
      console.log('[Workouts] fetched payload:', data);
  ...
  const filteredItems = items.filter((item) => {
    ...
    const text = `${item.title ?? ''} ${item.difficulty ?? ''} ${item.recommendation_notes ?? ''} ${item.id ?? ''}`
      .toLowerCase();
        ...
        <form className="row g-2 mb-3" onSubmit={(event) => event.preventDefault()}>
          <div className="col-sm-8 col-md-6">
            <input
              ...
              placeholder="Search workouts by title or difficulty"
              value={query}
              ...
              {loading ? (
                ...
                filteredItems.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>{item.recommendation_notes || '-'}</td>
