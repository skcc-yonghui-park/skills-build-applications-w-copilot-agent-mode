...
function Activities() {
  ...
  async function loadActivities(signal) {
    try {
      ...
      console.log('[Activities] fetched payload:', data);
  ...
  const filteredItems = items.filter((item) => {
    ...
    const text = `${item.activity_type ?? ''} ${item.duration_minutes ?? ''} ${item.calories_burned ?? ''} ${item.id ?? ''}`
      .toLowerCase();
        ...
        <form className="row g-2 mb-3" onSubmit={(event) => event.preventDefault()}>
          <div className="col-sm-8 col-md-6">
            <input
              ...
              placeholder="Search activities by type or metrics"
              value={query}
              ...
              {loading ? (
                ...
                filteredItems.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>{item.calories_burned} kcal</td>
